...
getExecName()
{
  ...
  if (ifn.endswith(".txt")) return ifn.drop_back(4).str();
  return (ifn + ".out").str();
}
...
int main(int argc, char** argv)
{
  ...
  int status = compileModule(module);
  if (status < 0) return -1;

  std::string executableName = getExecName();
  std::string command = "gcc " + OutputFilename + " -o " + executableName;
  std::string clean = "unlink " + OutputFilename;
...
getOutputStream()
{
  if (OutputFilename.empty()) {
    ...
    if (ifn.endswith(".txt"))
      OutputFilename = (ifn.drop_back(4) + ".o").str();
    else
      OutputFilename = (ifn + ".o").str();
  }

  std::error_code ec;
  auto out = llvm::make_unique<tool_output_file>(OutputFilename, ec, sys::fs::F_None);
  if (ec) {
    errs() << ec.message() << '\n';
    return nullptr;
  }
